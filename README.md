...
| mask_prompt_style | 'direct' or 'gaussian' | 'gaussian' (default) or 'direct' | Must match if using Phase 2 |
...
3. ✅ Use same `--mask_prompt_style` in Phase 2 and Phase 3 (e.g., both `direct`)
4. ✅ Use same `--use_roi_crop` and `--roi_expand_ratio` in both phases
5. ✅ Use `best_sam.pth` checkpoint (SAM-native format) for Phase 3
6. ✅ If skipping Phase 2: use `--mask_prompt_style gaussian` (default) for unfinetuned SAM
...
**Without Phase 2 (Direct E2E with Unfinetuned SAM) - GAUSSIAN STYLE:**
```bash
# Skip Phase 2, use original MedSAM directly
# IMPORTANT: Use gaussian style for unfinetuned SAM (matches SAMRefiner paper)
python scripts/train_e2e.py \
    --data_root ./dataset/processed \
    --datasets BUSI \
    --fold 0 \
...
**Why Gaussian for Unfinetuned SAM?**
The SAMRefiner paper shows that unfinetuned SAM works well with:
- **Gaussian-style mask prompts**: Blur creates softer boundaries matching SAM's training distribution
- **Positive + Negative point prompts**: Both are generated by default
- **Box prompts**: Extracted via soft-min/max

When SAM has been finetuned in Phase 2 with soft masks using `direct` style, Phase 3 should also use `direct` to maintain distribution consistency.

**Phase 3 Key Parameters:**
| Parameter | Value | Must Match Phase 2? |
|-----------|-------|---------------------|
| `--mask_prompt_style` | `gaussian` (unfinetuned SAM) or `direct` (Phase 2 finetuned) | ✓ YES (if using finetuned SAM) |
...
**Mask Prompt Style Quick Reference:**
| SAM Checkpoint | Recommended Style | Reason |
|----------------|-------------------|--------|
| Unfinetuned (MedSAM/SAM) | `gaussian` | Matches SAM's training; softens boundaries |
| Phase 2 finetuned (soft masks) | `direct` | Maintains distribution consistency |
| Phase 2 finetuned (binary masks) | `gaussian` | Adds blur to match Phase 2 |
